			...
			success:
				...
				if(data && data.text){
					...
					if ( updateContent !== '' ) {
					    ...
					    jQuery('#chat-area-'+pid).animate({ scrollTop: jQuery('#chat-area-'+pid).attr("scrollHeight") }, 2000);
					    jnpr_fixScroll(document.getElementById('chat-area-'+ pid));
					    if ( sounds !== 'disabled' && chat_localized['sound_'+pid] !== 'disabled' && pingSound ) {
						pingSound.play('notify');
					    }
					}
...
/**
 * Inserted 5/17/2012 by Michael A. Milazzo (mmilazzo@juniper.net)
 * 
 * The purpose of this function is to fix the scrolling behavior when a new chat message is added to the conversation.
 */
function jnpr_fixScroll(containingDiv) {
	containingDiv.scrollTop = 9999999; 
} 
